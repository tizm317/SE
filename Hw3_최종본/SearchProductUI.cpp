...
SearchProductUI::SearchProductUI(FILE* in_fp, FILE* out_fp, SearchProduct* searchProductptr, vector<Product*> productList)
{
	...
	fprintf(out_fp, "%s %s %s %d %d %.1f", ProductPointer->getSellerID().c_str(), ProductPointer->getProductName().c_str(), ProductPointer->getCompanyName().c_str(), ProductPointer->getCost(), ProductPointer->getCount(), std::roundf(ProductPointer->getAvgPurchaseSatisfaction()));
}
...
Product* SearchProductUI::startInterface(FILE* in_fp, FILE* out_fp, SearchProduct* searchProductptr, vector<Product*> productList)
{
	...
	fprintf(out_fp, "> %s %s %s %d %d %.1f\n", productPointer->getSellerID().c_str(), productPointer->getProductName().c_str(), productPointer->getCompanyName().c_str(), productPointer->getCost(), productPointer->getCount(), std::roundf(productPointer->getAvgPurchaseSatisfaction()));
	fprintf(out_fp, "\n");
